...
```tsx
function OverlayBackdrop({ close }: { close: (options: { strategy: "last" | "all" }) => void }) {
  return (
    <div
      aria-hidden="true"
...
Cue passes a `close` callback to the backdrop. The backdrop chooses whether to close the top open overlay with `{ strategy: "last" }` or every open overlay with `{ strategy: "all" }`.
