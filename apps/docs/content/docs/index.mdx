...
<Callout type="info">
With the registry install, import from `@/lib/cue` (or whatever `aliases.lib`
resolves to). That file re-exports `@vlkoss/cue`, so you can wrap or extend the
API in one place without forking the package.
</Callout>
