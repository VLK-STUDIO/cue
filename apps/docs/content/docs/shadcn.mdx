...
The backdrop creates a separate shadcn Dialog root and portal. That gives the native `DialogOverlay` the context it needs while Cue decides when the backdrop mounts. Cue passes the backdrop a `close` callback, and this adapter chooses to close the top open overlay. The overlay itself does not render a backdrop.
